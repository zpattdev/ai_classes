...
def test_word2vec():
    ...
    print("\nYour Result:")
    loss, dj_dv, dj_du = submission.skipgram(inputs['test_word2vec']['current_center_word'],
                                  inputs['test_word2vec']['outside_words'],
                                  dummy_tokens, dummy_vectors[:5, :], dummy_vectors[5:, :], dataset,
                                  submission.naive_softmax_loss_and_gradient)
    print(
        "Loss: {}\nGradient wrt Center Vectors (dJ/dV):\n {}\nGradient wrt Outside Vectors (dJ/dU):\n {}\n".format(loss,
                                                                                                                   dj_dv,
                                                                                                                   dj_du))
...
def hidden_test_sigmoid(x, soln, subm):
    points = 0
    student_result = subm.sigmoid(x.copy())

    soln_result = soln.sigmoid(x.copy())

    return np.allclose(student_result, soln_result, atol=1e-6)
...
def hidden_dummy(soln, subm):
    ...
    # use the solution's normalize rows
...
def hidden_test_softmax(soln, subm):
    ...
    student_loss, student_grad_center_vec, student_grad_outside_vecs = subm.naive_softmax_loss_and_gradient(
        center_word_vec,
        outside_word_idx,
        outside_vectors,
        dataset)
    ...
    return (np.isclose(student_loss, soln_loss, atol=1e-5), # cost does not match
            np.allclose(student_grad_center_vec, soln_grad_center_vec, atol=1e-5), # grad dj/v_c does not match
            np.allclose(student_grad_outside_vecs, soln_grad_outside_vecs, atol=1e-5) # grad dj/dU does not match
            )

def hidden_test_word2vec(soln, subm):
    student_model = subm.skipgram
    ...
    student_cost, student_grad_pred, student_grad = student_model(
        "c", window,
    ...
    return (np.isclose(student_cost, soln_cost, atol=1e-5), # cost does not match
            np.allclose(student_grad_pred, soln_grad_pred, atol=1e-5), # grad v_hat does not match
            np.allclose(student_grad, soln_grad, atol=1e-5) # grad vectors does not match
            )

def test_sgd(f, x, optimal):
  return np.allclose(
    submission.sgd(f, x, 0.01, 1000, PRINT_EVERY=10000),
    optimal, atol=1e-6)
...
class Test_2a(GradedTestCase):
  ...
  def test_3(self):
    ...
    self.assertTrue(hidden_test_sigmoid(np.array([-100, -3 - 2, -1, 0, 1, 2, 3, 100]),
                                        self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol), submission))
  ...
  def test_6(self):
    ...
    test_cost, test_grad_v_c, test_grad_U = hidden_test_softmax(self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol), submission)
    self.assertTrue(test_cost and test_grad_U and test_grad_v_c)

  @graded(is_hidden=True, timeout=10)
  def test_7(self):
    """2a-7-hidden:  test skipgram"""
    test_cost, test_grad_v_c, test_grad_U = hidden_test_word2vec(self.run_with_solution_if_possible(submission, lambda sub_or_sol:sub_or_sol), submission)
    self.assertTrue(test_cost and test_grad_U and test_grad_v_c)
...
class Test_2b(GradedTestCase):
  ...
  def test_0(self):
    ...
    t1 = submission.sgd(self.quad, 0.5, 0.01, 1000, PRINT_EVERY=100)
  ...
  def test_1(self):
    ...
    t2 = submission.sgd(self.quad, 0.0, 0.01, 1000, PRINT_EVERY=100)
  ...
  def test_2(self):
    ...
    t3 = submission.sgd(self.quad, -1.5, 0.01, 1000, PRINT_EVERY=100)
...
class Test_2c(GradedTestCase):
  ...
  def test_1(self):
    ...
    self.assertTrue(os.path.exists('sample_vectors_(soln).json'), 'Cannot run unit test because word vector file is not present.  It must be uploaded with your submission.py file.  Execute src/run.py to create the word vector file (sample_vectors_soln.json).')
    soln_vectors = load('sample_vectors_(soln)_compare.json')
    submission_vectors = load('sample_vectors_(soln).json')
    keys = soln_vectors.keys()
    n_vectors = len(keys)

    for key in keys:
      soln_vector = soln_vectors[key]
      submission_vector = submission_vectors.get(key, [0] * len(soln_vector))

      self.assertTrue(np.allclose(submission_vector, soln_vector, atol=1e-3))
